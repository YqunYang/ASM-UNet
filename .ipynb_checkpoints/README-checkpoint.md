...
nnUNetv2_train 1 3d_fullres all -tr nnUNetTrainer_asmunet_enc

# Train with multi-GPUs (example: 2 GPUs)
CUDA_VISIBLE_DEVICES=0,1 nnUNetv2_train 1 3d_fullres all -tr nnUNetTrainer_asmunet_enc -num_gpus 2
```

#### (Optional 1) Solution for Limited GPU Memory
In the file `nnUNetTrainer_asmunet_enc.py`, we provide an optional parameter `if_lmls` to handle cases where GPU memory is insufficient.
...
You may also choose to use the alternative trainer `nnUNetTrainer_asmunet`, which apply the ASM module in both the encoder and decoder.
