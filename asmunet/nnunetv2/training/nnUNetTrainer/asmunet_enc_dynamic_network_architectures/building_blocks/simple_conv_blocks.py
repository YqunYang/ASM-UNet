...
from .adaptive_scan_mamba import Adaptive_Scan_Mamba
...
class StackedConvBlocks(nn.Module):
    def __init__(self,
        ...
        if self.output_channels > 64:
            self.mamba = Adaptive_Scan_Mamba(self.output_channels, if_lmls=if_lmls, number_gs=3)
        self.initial_stride = maybe_convert_scalar_to_list(conv_op, initial_stride)

    @torch._dynamo.disable
    def _mamba_eager(self, x):
        return self.mamba(x)
    
    def forward(self, x):
        x = self.convs(x)
        if self.output_channels > 64:
            x = self._mamba_eager(x)
        return x
